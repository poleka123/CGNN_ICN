...
class Interactor(nn.Module):
    def __init__(self, in_planes, splitting=True,
                 kernel=5, dropout=0.5, groups=1, hidden_size=1, INN=True, dilation=1):
        ...
        modules_P += [
            ...
            nn.Conv2d(size_hidden, size_hidden,
                      kernel_size=(1, self.kernel_size), dilation=self.dilation, stride=1, groups=self.groups),
        ...
        modules_phi += [
            nn.ReplicationPad2d((pad_l, pad_r, 0, 0)),
            nn.Conv2d(in_planes, size_hidden,
                      kernel_size=(1, self.kernel_size), dilation=self.dilation, stride=1, groups=self.groups),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
            nn.Dropout(self.dropout),
            nn.Conv2d(size_hidden, size_hidden,
...
class LevelSCINet(nn.Module):
    ...
    def forward(self, x):
        ...
        return x_even_update, x_odd_update  # even: B, T, D odd: B, T, D
...
class SCINet(nn.Module):
    def __init__(self, output_len, input_len, input_dim=9, hid_size=1, num_stacks=1,
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
        # self.projection1 = nn.Conv2d(self.input_dim, self.input_dim, kernel_size=(1, 1), stride=1, bias=False)
        self.projection1 = nn.Linear(self.input_dim*self.input_len, self.output_len)
    ...
    def get_position_encoding(self, x):
        ...
        signal = signal.view(1, self.pe_hidden_size, max_length, 1)
    ...
    def forward(self, x):
        ...
        # the first stack
        res1 = x.permute(0, 3, 1, 2)
        x = self.blocks1(x.permute(0, 3, 1, 2))
        x += res1
        if self.num_decoder_layer == 1:
            x = self.projection1(x.reshape(x.shape[0], x.shape[2], x.shape[1]*x.shape[3]))
        else:
...
class InteractorandConcat(nn.Module):
    def __init__(self, in_planes, kernel, dropout, groups, hidden_size, INN, dilation):
        super(InteractorandConcat, self).__init__()
        self.level = Interactor(in_planes=in_planes, splitting=True,
                                kernel=kernel, dropout=dropout, groups=groups, hidden_size=hidden_size, INN=INN, dilation=dilation)
        self.relu = nn.ReLU()
        self.downsample = nn.Conv2d(in_planes, hidden_size, (1, 1))
    ...
    def forward(self, x):
        ...
        return self.relu(x_concat + res)

class InterConvNet(nn.Module):
    def __init__(self, output_len, input_len, input_dim, hid_size, num_stacks=1,
                 num_levels=3, num_decoder_layer=1, concat_len=0, groups=1, kernel=5, dropout=0.5,
                 single_step_output_One=0, input_len_seg=0, positionalE=False, modified=True, RIN=False):
        super(InterConvNet, self).__init__()
        self.input_dim = input_dim
        self.input_len = input_len
        self.output_len = output_len
        self.hidden_size = hid_size
        ...
        #     input_channel = input_dim if i==0 else hid_size
        #     dilation_size = 2**i
        #     layer += [InteractorandConcat(input_channel, kernel, dropout, groups, hid_size, modified, dilation_size)]
        # self.network = nn.Sequential(*layer)

        # tcn ******
        hidden = [32, 32, 32]
        self.network = TemporalConvNet(input_dim, hidden, kernel)

        self.projection1 = nn.Linear(self.hidden_size * self.input_len, self.output_len)

    def forward(self, x):
        x_update = self.network(x.permute(0, 3, 1, 2))
        x_update = self.projection1(x_update.reshape(x_update.shape[0], x_update.shape[2], x_update.shape[1]*x_update.shape[3]))
        return x_update
