...
class GaussConvNet(nn.Module):
    def __init__(self, num_inputs, num_channels, g_kernel, Ks=3, kernel_size=2, dropout=0.2):
        super(GaussConvNet, self).__init__()
        layer = []
        gcn_layer = []
        self.tcn_conv = nn.ModuleList()
        ...
        for i in range(num_levels):
            ...
            self.tcn_conv.append(GaussBlack(in_channels, out_channels, g_kernel, kernel_size, stride=1, dilation=dilation_size,
                                 padding=(0, (kernel_size-1)*dilation_size), dropout=dropout))
            self.gcn_conv.append(SpatioConvBlack(Ks, out_channels))
    ...
    def forward(self, Lk, x):
        residual = x
        for i in range(self.layers):
            x = self.tcn_conv[i](x)
            x = self.gcn_conv[i](x, Lk)
...
class GTCN(nn.Module):
    def __init__(self, FFT_data, Ks, input_size, input_timesteps, output_size, num_channels, g_kernel, kernel_size, dropout, device):
        ...
        self.GTCN = GaussConvNet(input_size, num_channels, g_kernel=g_kernel, Ks=Ks, kernel_size=kernel_size, dropout=dropout)
        # gass
